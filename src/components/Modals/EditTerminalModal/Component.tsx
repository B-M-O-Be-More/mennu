...
export default function EditTerminalModal({ open, onClose, terminal, onSave }: EditTerminalModalProps) {
  ...
  React.useEffect(() => {
    if (!open || !terminal?.id) return;
    setIsLoading(true);

    const fetchTerminalDetail = fetch(`/api/terminais/${terminal.id}`)
      .then((r) => r.json())
      .then((resp) => resp?.data ?? resp)
      .catch(() => null);

    const fetchUnidades = fetch("/api/unidades/")
      .then((r) => r.json())
      .then((resp) => normalizeList<{ id: number; nome: string }>(resp))
      .catch(() => [] as { id: number; nome: string }[]);

    const fetchRefeicoes = fetch("/api/tipo-refeicao/")
      .then((r) => r.json())
      .then((resp) => normalizeList<{ id: number; nome: string }>(resp))
      .catch(() => [] as { id: number; nome: string }[]);

    Promise.all([fetchTerminalDetail, fetchUnidades, fetchRefeicoes])
      .then(([detail, uns, refs]) => {
        setUnidades(uns.map((u) => ({ id: u.id, nome: u.nome })));
        setTiposRefeicao(refs.map((r) => ({ id: r.id, nome: r.nome })));
        reset({
          nome: detail?.nome ?? terminal.nome ?? "",
          tipo: detail?.tipo ?? terminal.tipo ?? "",
          unidade_id: detail?.unidade_id ?? undefined,
          refeicoes_permitidas: (detail?.refeicoes_permitidas ?? []).map(Number),
          categorias_permitidas: detail?.categorias_permitidas ?? [],
  ...
  const onSubmit = async (data: CreateTerminalFormData) => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/terminais/${terminal.id}/`, {
        method: "PUT",
      ...
      if (!response.ok) {
        ...
        throw new Error((err as { message?: string }).message ?? "Erro ao atualizar terminal");
      }
      setToast({ open: true, message: "Terminal atualizado com sucesso!", severity: "success" });
      onSave();
      onClose();
    } catch (err: unknown) {
      setToast({ open: true, message: (err as Error).message ?? "Erro ao atualizar terminal.", severity: "error" });
    } finally {
      setIsLoading(false);
    }
  };
